...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 26 08:17:28 2020

@author: ChrisTokita

SCRIPT:
Grab friend/follower network for late arriving participants
"""
...
####################
# Set important paths and paramteres
####################
...
late_worker_files = ['survey_data/MTurk_workers/workers_rd2_highcorr_final2.csv',
                     'survey_data/MTurk_workers/workers_rd2_lowcorr_final2.csv']


late_workers = []
for file in late_worker_files:
    _, res = dbx.files_download(dropbox_dir + file)
    with io.BytesIO(res.content) as stream:
        workers = pd.read_csv(stream)
        late_workers.append(workers)
        del workers
late_workers = pd.concat(late_workers)


####################
# Determine which user IDs to lookup
####################
...
# Only lookup users who did the survey after Dec 20 (I pulled later that night)
late_workers['SubmitTime'] = pd.to_datetime(late_workers['SubmitTime'])
late_workers = late_workers[late_workers.SubmitTime > datetime.datetime(year = 2020, month = 12, day = 20, hour = 13)] #time is PST, I started at 4pm EST
user_crosswalk = user_crosswalk[user_crosswalk['mturk_worker_id'].isin(late_workers['WorkerId'])]
