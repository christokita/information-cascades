...
def construct_initial_network(user_ids, initial_friendfollower_files):
    ...
    for user in user_ids:
        ...
        user_files = [x for x in initial_friendfollower_files if re.search(user, x)]
...
############################### Final follow networks ###############################

####################
# Construct final network
####################
# Function to construct initial networks
def construct_final_network(user_ids, final_friendfollower_files):
    ...
    for user in user_ids:
        ...
        # Grab friend/follower files
        user_files = [x for x in final_friendfollower_files if re.search(user, x)]
...
# Combine friend and follower lists into one to make parsing easier, narrow to just the date in which we pulled final follower lists
friendfollower_files = follower_list_files + friend_list_files
final_friendfollower_files = [x for x in friendfollower_files if re.search('2021-01-20', x)]

# Construct networks
lowcorr_final_network, lowcorr_final_follower_network = construct_final_network(user_ids = low_corr_users['user_id'], final_friendfollower_files = final_friendfollower_files)
highcorr_final_network, highcorr_final_follower_network = construct_final_network(user_ids = high_corr_users['user_id'], final_friendfollower_files = final_friendfollower_files)

# Determine which users we do not have lists for (for accounts that were Protected when we collected friend/follower lists)
unique_users_with_data = [re.search("[a-z]+_([0-9]+)_.*", x).group(1) for x in final_friendfollower_files]
unique_users_with_data = np.unique(unique_users_with_data)
lowcorr_users_without_list = [x for x in low_corr_users['user_id'] if x not in unique_users_with_data] #only one user
highcorr_users_without_list =  [x for x in high_corr_users['user_id'] if x not in unique_users_with_data] #only one user

# Fill in missing data
for user_without_list in lowcorr_users_without_list:
    lowcorr_final_network.loc[user_without_list, :] = lowcorr_final_follower_network[user_without_list]
    
for user_without_list in highcorr_users_without_list:
    highcorr_final_network.loc[user_without_list, :] = highcorr_final_follower_network[user_without_list]
...
# Write files to temporary directory
lowcorr_final_network.to_csv(tmp_dir + 'lowcorr_final_network.csv', index = True)
highcorr_final_network.to_csv(tmp_dir + 'highcorr_final_network.csv', index = True)

# Upload to dropbox
for file in ['lowcorr_final_network.csv', 'highcorr_final_network.csv']:
    with open(tmp_dir + file, "rb") as f:
        dbx.files_upload(f.read(), path = path_to_twitter_data + file, mode = dropbox.files.WriteMode.overwrite)  
