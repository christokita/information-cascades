...
# Load data
users_ideology <- read.csv('empirical/data_derived/monitored_users/monitored_users_ideology_scores.csv', colClasses = c("user_id"="character")) %>% 
  mutate(user_id = gsub("\"", "", user_id_str))
users <- read.csv('empirical/data_derived/monitored_users/monitored_users_preliminary.csv', colClasses = c("user_id"="character")) %>% 
  mutate(user_id = gsub("\"", "", user_id_str))
...
ggsave(gg_ideology_hists, filename = "empirical/output/ideology/ideology_distribution_by_method.png", width = 90, height = 90, units = "mm", dpi = 400)
...
ggsave(gg_compare_ideology_scores, filename = "empirical/output/ideology/compare_estimation_methods.png", width = 45, height = 45, units = "mm", dpi = 400)
...
ggsave(gg_ideology_by_outlet, filename = "empirical/output/ideology/method_vs_newssource.png", width = 100, height = 60, units = "mm", dpi = 400)
