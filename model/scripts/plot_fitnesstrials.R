...
gg_activity <- ggplot(activity, aes(x = trial, y = mean, color = gamma, group = gamma)) +
  ...
  scale_y_continuous(breaks = seq(0, 100, 10), limits = c(10, 60), expand = c(0, 0)) +
  ylab("Total cascade activity") +
  theme_ctokita() +
  theme(axis.title.x = element_blank(),
        legend.position = "none")
gg_activity #show plot before saving
ggsave(plot = gg_activity, filename = paste0(out_path, "cascadeactivity", plot_tag ,".pdf"), width = 45, height = 45, units = "mm", dpi = 400)
...
ggsave(plot = gg_size, filename = paste0(out_path, "cascadesize", plot_tag ,".pdf"), width = 45, height = 45, units = "mm", dpi = 400)
...
ggsave(plot = gg_bias, filename = paste0(out_path, "cascadebias", plot_tag ,".pdf"), width = 45, height = 45, units = "mm", dpi = 400)
...
ggsave(plot = gg_behavrates, filename = paste0(out_path, "behaviorrates", plot_tag ,".pdf"), width = 90, height = 90, units = "mm", dpi = 400)
...
gg_sens <- ggplot(sensitivity, aes(x = trial, y = mean, color = gamma, group = gamma)) +
  ...
  scale_color_gradientn(colors = pal, name = expression(paste("Information\necosystem", gamma))) +
  scale_x_discrete(labels = c("Initial network", "Final network")) +
  scale_y_continuous(breaks = seq(0, 1, 0.05), limits = c(0.1, 0.4), expand = c(0, 0)) +
  ylab("Sensitivity") +
  theme_ctokita() +
  theme(axis.title.x = element_blank(),
        legend.position = "none") +
  coord_cartesian(clip = "off") #prevent points from being clipped when on the end of an axis range
...
gg_spec <- ggplot(specificity, aes(x = trial, y = mean, color = gamma, group = gamma)) +
  ...
  scale_color_gradientn(colors = pal, name = expression(paste("Information\necosystem", gamma))) +
  scale_x_discrete(labels = c("Initial network", "Final network")) +
  scale_y_continuous(breaks = seq(0, 1, 0.04), limits = c(0.76, 1), expand = c(0, 0)) +
  ylab("Specificity") +
  theme_ctokita() +
  theme(axis.title.x = element_blank(),
        legend.position = "none")  +
...
gg_prec <- ggplot(precision, aes(x = trial, y = mean, color = gamma, group = gamma)) +
  ...
  scale_color_gradientn(colors = pal, name = expression(paste("Information\necosystem", gamma))) +
  scale_x_discrete(labels = c("Initial network", "Final network")) +
  scale_y_continuous(breaks = seq(0, 1, 0.02), limits = c(0.88, 1), expand = c(0, 0)) +
  ylab("Precision") +
  theme_ctokita() +
  theme(axis.title.x = element_blank(),
        legend.position = "none")
