########################################
#
# PLOT: Network structure given identical thresholds and no new tie formation, for different simulation run time.
#
########################################
...
# Identical thresholds, 10^6 steps
...
# Identical thresholds, 10^7 steps
iden_longer_data <- read.csv('data_derived/network_break/__suppl_analysis/identical_thresholds_p0_longersim/social_networks/assortativity_identicalthresh_p0_10^7steps.csv', header = TRUE) %>% 
  mutate(threshold_dist = "Identical_10^7steps")

# Bind
assort_sum <- rbind(iden_data, iden_long_data, iden_longer_data) %>% 
  mutate(delta_assort = assort_final - assort_initial,
         threshold_dist = factor(threshold_dist, levels = c("Identical_10^7steps", "Identical_10^6steps", "Identical"))) %>% 
  select(-replicate) %>% 
...
rm(iden_data, iden_long_data, iden_longer_data)
...
ggsave(plot = gg_assort_threhsolds, 
       filename = "output/network_break/__suppl_analysis/p0/assortativity_by_identicalthresh_nonewties.png", 
       height = 45, 
